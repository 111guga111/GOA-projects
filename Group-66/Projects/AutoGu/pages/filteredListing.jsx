...
function FilteredListing() {
  const { filteredCars, handleRemoveFilteredCar } = useFilteredCars();
  const { cart, handleAdd, handleRemove, handleIncrease, handleDecrease } = useCart();
  const { handleRemoveAddedCar } = useAdmin();
  const {user} = useAuth();

  return (
    <div className="flex flex-col items-center gap-6 mt-5 min-h-screen">
      {filteredCars.length === 0 ? (
        <p className="text-white text-lg">No cars found matching your filter, try again.</p>
      ) : (
              ...
              >
                {user.admin && car.seller && (
                    <button
                        ...
                        onClick={() => {handleRemoveAddedCar(car); handleRemoveFilteredCar(car)}}
                    />
                )}
                <img
                  src={car.imageURL}
